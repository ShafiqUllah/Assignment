...
public class Main {
	...
	public static List<String> allWhoseZipContains3() {
		...
		mems.addAll(members);
		//implement
		return mems.stream().map(m -> m.getAddress().getZip()).filter(x -> x.contains("3")).collect(Collectors.toList());
	...
	public static List<String> allHavingMultipleAuthors() {
		...
		bs.addAll(books);
		//implement
		return bs.stream().filter(x -> x.getAuthors().size() > 1).map(x -> x.getIsbn()).collect(Collectors.toList());
