...
public class MyBST {
	...
	private Node root;
	public int[] arr;
	private static ArrayList<Integer> bList = new ArrayList<>();
...
//				return 0;
//
//			return BTreePrinter.maxLevel(node.left) + BTreePrinter.maxLevel(node.right) + 1;
//		}

		static void recSizeCalculator(Node t) {
			if (t != null) {
				recSizeCalculator(t.left);
				BTreePrinter.aList.add(t.element);
				recSizeCalculator(t.right);
			}
		}

		static int size(Node t) {
			BTreePrinter.aList.clear();
			recSizeCalculator(t);
			return BTreePrinter.aList.size();
		}
